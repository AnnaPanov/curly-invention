...
namespace Recipes
{
    public class PaleoLeapRecipeDownloader : RecipeDownloader
    {
        public PaleoLeapRecipeDownloader(string nextUrl, string path) : base(nextUrl, path, "PaleoLeap", "http://paleoleap.com") { }
        ...
        protected override void DownloadRawPage(string url, out string title, out string text, out HashSet<string> referencedUrls)
        {
            using (WebClient client = new System.Net.WebClient())
            {
                ...
                text = client.DownloadString("http://paleoleap.com" + url);
